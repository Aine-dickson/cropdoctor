  ...
  <div class="view">
    ...
    <div class="page">
      ...
      <div class="section">
        ...
        <input
          v-if="form.payment === 'mtn'"
          class="field" v-model="form.momoNumber"
          placeholder="MTN number e.g. 0771234567" type="tel"
        />
        <input
          v-if="form.payment === 'airtel'"
          class="field" v-model="form.momoNumber"
      ...
      <div class="section">
        ...
        <div class="summary-card">
          ...
          <div class="mini-row muted"><span>Delivery</span><span>UGX {{ cart.DELIVERY_FEE.toLocaleString() }}</span></div>
          <div class="mini-row total"><span>Total</span><span>UGX {{ cart.total.toLocaleString() }}</span></div>
        </div>
      </div>
...
.page { flex: 1; padding: 20px 16px 100px; max-width: 480px; width: 100%; margin: 0 auto; display: flex; flex-direction: column; gap: 20px; }
.section { display: flex; flex-direction: column; gap: 10px; }
.section-label { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--muted); }
.field { width: 100%; font-family: 'Sora', sans-serif; font-size: 16px; background: var(--surface); border: 2px solid var(--border); border-radius: 12px; padding: 16px 14px; color: var(--text); transition: border-color 0.2s; -webkit-appearance: none; }
.field:focus { outline: none; border-color: var(--accent); }
.field::placeholder { color: var(--muted); }
.payment-list { display: flex; flex-direction: column; gap: 10px; }
.payment-opt { display: flex; align-items: center; gap: 14px; background: var(--surface); border: 2px solid var(--border); border-radius: 14px; padding: 16px; cursor: pointer; min-height: 68px; -webkit-tap-highlight-color: transparent; touch-action: manipulation; transition: border-color 0.15s, background 0.15s; }
.payment-opt.selected { border-color: var(--accent); background: var(--accent-lt); }
.pay-logo { width: 48px; height: 34px; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 800; flex-shrink: 0; }
.pay-logo.mtn    { background: #ffcc00; color: #000; }
.pay-logo.airtel { background: #e40000; color: #fff; }
.pay-logo.cod    { background: var(--surface2); font-size: 20px; border: 1px solid var(--border); }
.pay-name { font-size: 14px; font-weight: 600; }
.pay-hint { font-size: 12px; color: var(--muted); margin-top: 1px; }
.pay-check { margin-left: auto; color: var(--accent); font-size: 18px; font-weight: 700; }
.summary-card { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 16px; display: flex; flex-direction: column; gap: 10px; }
.mini-row { display: flex; justify-content: space-between; font-size: 13px; color: var(--text); }
.mini-row.muted { color: var(--muted); }
.mini-row.total { font-size: 16px; font-weight: 700; padding-top: 10px; border-top: 1px solid var(--border); margin-top: 2px; }
