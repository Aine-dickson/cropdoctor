    ...
    <div class="view">
        ...
        <div v-else class="page">
            <div class="hero-band">
                ...
                <span class="hero-stock" :class="stockClass">{{ stockLabel }}</span>
            </div>
            ...
            <div class="body-card">
                ...
                <p class="stock-note" :class="stockClass">{{ stockLabel }}</p>

                <div class="meta-grid">
                    <div class="meta-cell">
                        <p class="meta-label">Treats</p>
                        <p class="meta-val">{{ product.treats }}</p>
                    </div>
                    <div class="meta-cell">
                        <p class="meta-label">Application</p>
                        <p class="meta-val">{{ product.application }}</p>
                    </div>
                    <div class="meta-cell">
                        <p class="meta-label">Dosage</p>
                        <p class="meta-val">{{ product.dosage }}</p>
                    </div>
                    <div class="meta-cell">
                        <p class="meta-label">Supplier</p>
                        <p class="meta-val">{{ product.supplier }}</p>
                    </div>
                </div>
                ...
                <div class="qty-row">
                    ...
                    <div class="qty-control">
                        <button class="qty-btn" :disabled="!isAvailable || qty <= 1"
                            @click="qty = Math.max(1, qty - 1)">−</button>
                        <span class="qty-num">{{ qty }}</span>
                        <button class="qty-btn" :disabled="!isAvailable || qty >= availableStock"
                            @click="qty++">+</button>
                    </div>
...
<script setup>
    ...
    const availableStock = computed(() => (product.value ? product.value.inventoryCount : 0))
    const isAvailable = computed(() => !!product.value && isMedicineInStock(product.value.id))
    const stockLabel = computed(() => (product.value ? getMedicineStockLabel(product.value.id) : 'Out of stock'))
    const stockClass = computed(() => (product.value ? getLowStockBadge(product.value.id) : 'out-of-stock'))
...
<style scoped>
    ...
    .hero-stock.low-stock {
        background: #fef3e2;
        color: var(--warn);
    }

    .hero-stock.out-of-stock {
        background: #fde8e8;
        color: var(--danger);
    }
