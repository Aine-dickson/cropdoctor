  ...
  <nav class="bottom-bar">
    ...
    >
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        ...
        <path d="M11 3v2M11 17v2M3 11H1M21 11h-2" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" opacity=".5"/>
      </svg>
      <span>Scan</span>
    </button>
    ...
    <button
      class="tab"
      :class="{ active: route.path === '/describe' }"
    ...
    >
      <template v-if="auth.isLoggedIn">
        <span class="avatar-tab">{{ auth.displayName[0].toUpperCase() }}</span>
      </template>
      <template v-else>
        <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
          <circle cx="11" cy="8" r="4" stroke="currentColor" stroke-width="1.7"/>
          <path d="M3 20c0-4.418 3.582-8 8-8s8 3.582 8 8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
        </svg>
      </template>
      <span>Account</span>
    </button>
